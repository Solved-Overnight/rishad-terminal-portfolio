...
import { ColorizedOutput } from '../utils/formatTerminalOutput';
            ...
            {history.length === 0 ? (
                ...
                history.slice().reverse().map((item, index) => (
                    <div key={index} className="border-b border-terminal-dim/30 last:border-0 pb-3">
                         ...
                         <div className="pl-5 text-terminal-text-dim whitespace-pre-wrap break-words text-xs leading-relaxed opacity-90 border-l-2 border-terminal-dim/30 ml-1">
                            {item.output ? <ColorizedOutput text={item.output} /> : <span className="italic opacity-50">No output recorded</span>}
                         </div>
                    </div>
                ))
            )}
