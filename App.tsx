...
const PortfolioOS: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="md:hidden flex-none h-14 bg-terminal-black/90 backdrop-blur-md border-b border-terminal-dim flex items-center justify-between px-2.5 sm:px-4 z-40 sticky top-0 gap-2 relative">
          {/* Left: Mobile Buttons */}
          <div className="flex items-center gap-1.5 flex-shrink-0 z-10">
            <button
                onClick={() => { setSelectedBlogId(null); setIsBlogOpen(true); }}
                className="font-mono text-[11px] text-terminal-primary bg-terminal-primary/10 hover:bg-terminal-primary hover:text-terminal-black transition-all flex items-center gap-1 cursor-pointer py-1 px-2 rounded-md font-semibold flex-shrink-0"
                title="Read Articles & Blogs (File Explorer)"
            >
                <span>Blogs</span>
            ...
            <button
                ...
                className="font-mono text-[11px] text-cyan-400 bg-cyan-500/10 hover:bg-cyan-500 hover:text-black transition-all flex items-center gap-1 cursor-pointer py-1 px-2 rounded-md font-semibold flex-shrink-0 border border-cyan-500/30"
                title="Projects Showcase (Interactive Stack)"
            >
                <span>Projects</span>
                <span className="text-[10px]">↗</span>
            </button>
          ...
          <div className="flex bg-terminal-dim/30 rounded-lg p-0.5 border border-terminal-dim/40 flex-shrink-0 absolute left-1/2 -translate-x-1/2 z-10">
             <button 
                onClick={() => setActiveMobileTab('profile')}
                className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider transition-all ${activeMobileTab === 'profile' ? 'bg-terminal-primary text-terminal-black shadow-sm' : 'text-terminal-text-dim hover:text-terminal-text'}`}
             >
                ID Card
             </button>
             <button 
                onClick={() => setActiveMobileTab('terminal')}
                className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider transition-all ${activeMobileTab === 'terminal' ? 'bg-terminal-primary text-terminal-black shadow-sm' : 'text-terminal-text-dim hover:text-terminal-text'}`}
             >
                Terminal
             </button>
          </div>
          ...
          <div className="flex items-center gap-1 flex-shrink-0 z-10">
            ...
            <button 
                ...
                className="text-terminal-text-dim hover:text-terminal-primary p-1.5 rounded-md hover:bg-terminal-dim/20 transition-colors"
            ...
            <div className="relative" ref={mobilePowerRef}>
                <button 
                    ...
                    className="text-terminal-text-dim hover:text-red-500 p-1.5 rounded-md hover:bg-terminal-dim/20 transition-colors"
                    title="Power Options"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div 
        className={`
            relative flex-shrink-0 bg-gradient-to-b from-gray-500 via-[#181a1d] to-[#080809] flex flex-col items-center 
      ...
      >
        ...
        >
            ...
            <button 
              onClick={() => setIsProjectsOpen(true)}
              className="p-2 text-terminal-text-dim hover:text-cyan-400 hover:scale-110 transition-all duration-300 rounded-full hover:bg-terminal-dim/20"
              title="Projects Showcase (Interactive Stack)"
            >
        ...
        <div 
            className={`
                ...
                bg-terminal-black/90 backdrop-blur-md px-3.5 py-1.5 rounded-full shadow-2xl 
                flex flex-row items-center justify-center gap-3 max-w-[95vw]
                md:absolute md:bottom-6 md:left-6 md:translate-x-0 md:z-20 md:bg-transparent md:backdrop-blur-none md:border-none md:p-0 md:shadow-none md:max-w-none
                transition-all duration-500 ease-in-out
                ${isSidebarCompact ? 'md:flex-col md:gap-3' : 'md:flex-row md:gap-4'}
            `}
        >
            <div className={`flex flex-row gap-2.5 sm:gap-3 items-center ${isSidebarCompact ? 'md:flex-col' : ''}`}>
                {socialLinks.map((social) => (
                    <a 
                        ...
                        className="w-8 h-8 rounded-full border-none md:border md:border-terminal-dim flex items-center justify-center text-terminal-text-dim hover:text-terminal-primary hover:border-terminal-primary transition-all bg-terminal-black/50 hover:bg-terminal-dim/30 flex-shrink-0"
                        title={social.name}
                    >
            ...
            <StarBorder 
                ...
                className={`transition-all duration-300 flex-shrink-0 ${isSidebarCompact ? 'md:w-8 md:h-8 md:p-0' : ''}`}
                title="Contact Me"
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <span className={`${isSidebarCompact ? 'md:hidden' : 'block'} text-xs font-semibold whitespace-nowrap`}>Contact Me</span>
            </StarBorder>
        </div>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 h-8 bg-terminal-black border-t border-terminal-dim flex items-center justify-between px-4 text-sm z-30 select-none">
            <div className="hidden md:flex items-center gap-4">
              <button
                  onClick={() => { setSelectedBlogId(null); setIsBlogOpen(true); }}
                  className="flex items-center gap-1 font-mono text-xs text-terminal-text-dim hover:text-terminal-primary transition-colors cursor-pointer group py-0.5"
                  title="Click to open Articles & Blogs"
              >
                  <span className="relative">
                      Read Blogs
                      <span className="absolute bottom-0 left-0 w-0 h-[1px] bg-terminal-primary transition-all duration-300 group-hover:w-full"></span>
                  </span>
                  <span className="text-[10px] transition-transform duration-300 group-hover:translate-x-0.5">↗</span>
              </button>
            </div>
            <span className="text-terminal-primary text-xs md:text-sm mx-auto">{formattedTime}</span>
            <a 
                href="#" 
                className="hidden md:flex items-center gap-1 font-mono text-xs text-terminal-text-dim hover:text-terminal-primary transition-colors group py-0.5"
            >
               <span className="relative">
                   Featured on Solved Overnight
                   <span className="absolute bottom-0 left-0 w-0 h-[1px] bg-terminal-primary transition-all duration-300 group-hover:w-full"></span>
               </span>
               <span className="text-[10px] transition-transform duration-300 group-hover:translate-x-0.5">↗</span>
            </a>
      ...
      <BlogModal 
        isOpen={isBlogOpen} 
        onClose={() => { setIsBlogOpen(false); setSelectedBlogId(null); }} 
